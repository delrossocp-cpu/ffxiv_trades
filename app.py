...
def get_trades():
    conn = get_db()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM public.trades ORDER BY created_at DESC")
    trades = cur.fetchall()
    cur.close()
    conn.close()
...
def update_trade(trade_id):
    ...
    cur.execute("SELECT quantity, total_purchase_price FROM public.trades WHERE id = %s", (trade_id,))
    trade = cur.fetchone()
...
def add_trade():
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO public.trades (item_id, item_name, quantity, purchase_price_per_unit,
